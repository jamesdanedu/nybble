...
const STEP_PUBLIC_FIELDS = ['id', 'runner_id', 'title', 'weight', 'config', 'allowResubmit'];
...
export function splitStep(raw) {
  ...
  // Only `true` is worth storing: answer-once is the default, and the scorer
  // treats anything other than exactly true as "no".
  if (raw.allowResubmit === true) step.allowResubmit = true;
 ...
 * @param {{ stepId?: string, earlierStepIds?: Set<string>, hasContextCode?: boolean }} [ctx]
 *        The step's own id, the ids of every step before it (so a runner that
 *        references another step — freetext's `showPrior` — can be checked
 *        against the activity it actually sits in), and whether the activity's
 *        shared_context carries code for a runner to fall back on.
 */
function validateRunnerStep(runnerId, config, key, path, errors, warnings, ctx = {}) {
  ...
  if (runnerId === 'freetext') {
    if (typeof config.prompt !== 'string' || !config.prompt.trim()) {
      err(`${path}.config.prompt`, 'freetext needs a prompt telling the student what to write about.');
    }

    for (const field of ['rows', 'minChars', 'maxChars']) {
      const v = config[field];
      if (v !== undefined && (typeof v !== 'number' || !Number.isFinite(v) || v < 0)) {
        err(`${path}.config.${field}`, `${field} must be a non-negative number.`);
      }
    }
    const min = typeof config.minChars === 'number' ? config.minChars : 0;
    const max = typeof config.maxChars === 'number' ? config.maxChars : 0;
    if (min && max && min > max) {
      err(`${path}.config.minChars`, `minChars (${min}) is above maxChars (${max}), so nothing can be submitted.`);
    }

    // `showPrior` is how a Make step quotes a Predict answer back. A typo or a
    // forward reference renders nothing at all, silently, in front of a
    // student — so it is checked here rather than discovered in a classroom.
    if (config.showPrior !== undefined) {
      const want = config.showPrior;
      if (!isObject(want)) {
        err(`${path}.config.showPrior`, 'showPrior must be an object like { "stepId": "predict" }.');
      } else if (typeof want.stepId !== 'string' || !want.stepId) {
        err(`${path}.config.showPrior.stepId`, 'showPrior needs the id of the step to quote back.');
      } else if (want.stepId === ctx.stepId) {
        err(`${path}.config.showPrior.stepId`, 'A step cannot quote itself back.');
      } else if (ctx.earlierStepIds && !ctx.earlierStepIds.has(want.stepId)) {
        err(
          `${path}.config.showPrior.stepId`,
          `"${want.stepId}" is not an earlier step in this activity. A step only ever sees the ones before it.`,
        );
      }
      if (isObject(want) && want.field !== undefined && typeof want.field !== 'string') {
        err(`${path}.config.showPrior.field`, 'field must be the name of a key in that step\'s response.');
      }
    }
    ...
    if (Object.keys(key).length) {
      warn(
        `${path}.key`,
        'freetext is marked by a teacher and has no key; this one is stored but never read.',
      );
    }
    return;
  }
  ...
  if (runnerId === 'pyrun') {
    // The program can come from config.source or from the activity's
    // shared_context.code. With neither there is nothing to run.
    const hasSource = typeof config.source === 'string' && config.source.trim();
    if (!hasSource && !ctx.hasContextCode) {
      err(
        `${path}.config.source`,
        'pyrun has no program: give it a source, or put code in the activity\'s shared_context.',
      );
    }

    if (config.execLimitMs !== undefined &&
        (typeof config.execLimitMs !== 'number' || !(config.execLimitMs > 0))) {
      err(`${path}.config.execLimitMs`, 'execLimitMs must be a positive number of milliseconds.');
    }
    if (config.stdin !== undefined &&
        (!Array.isArray(config.stdin) || config.stdin.some((l) => typeof l !== 'string'))) {
      err(`${path}.config.stdin`, 'stdin must be an array of strings, one answer per input() call.');
    }

    // Tests are PUBLIC on purpose, and there is no alternative: the runner
    // executes them in the browser and a runner is never sent the key. Say so
    // plainly, because "secrets go under key" is the rule everywhere else here
    // and a teacher following it would get tests that silently never run.
    if (isObject(key) && key.tests !== undefined) {
      err(
        `${path}.key.tests`,
        'pyrun tests must go under config, not key. They are run in the student\'s browser, ' +
        'and a runner is never sent the key — tests here would never run at all.',
      );
    }
    ...
    if (list !== undefined) {
      if (!Array.isArray(list)) {
        err(`${path}.config.tests`, 'tests must be an array.');
      } else {
        const seen = new Set();
        list.forEach((t, i) => {
          const tp = `${path}.config.tests[${i}]`;
          if (!isObject(t) || typeof t.id !== 'string' || !t.id) {
            err(`${tp}.id`, 'Every test needs a string id.');
            return;
          }
          if (seen.has(t.id)) err(`${tp}.id`, `Duplicate test id "${t.id}".`);
          seen.add(t.id);

          const isCall = typeof t.call === 'string' && t.call.trim();
          const isStdout = t.stdout !== undefined;
          if (isCall && isStdout) {
            err(`${tp}`, 'A test checks either a value (call/expect) or the whole output (stdout), not both.');
          } else if (!isCall && !isStdout) {
            err(`${tp}`, 'A test needs either call + expect, or stdout.');
          }
          if (isCall && typeof t.expect !== 'string') {
            err(`${tp}.expect`, 'A call test needs expect: the repr() of the value, as a string — "4", "[1, 2]", "\'hi\'".');
          }
          if (isStdout && typeof t.stdout !== 'string') {
            err(`${tp}.stdout`, 'stdout must be a string: exactly what the program should print.');
          }
          if (t.marks !== undefined && (typeof t.marks !== 'number' || t.marks < 0)) {
            err(`${tp}.marks`, 'marks must be a non-negative number.');
          }
        });
      }
    }

    if (Object.keys(key).length && key.tests === undefined) {
      warn(`${path}.key`, 'pyrun has no key; this one is stored but never read.');
    }
    return;
  }
...
export function parseActivityFile(raw) {
  ...
  list.forEach((a, ai) => {
    ...
    rawSteps.forEach((s, si) => {
      ...
      if (s.allowResubmit !== undefined && typeof s.allowResubmit !== 'boolean') {
        err(`${sp}.allowResubmit`, 'allowResubmit must be true or false.');
      }
      ...
      // seenStepIds already holds this step's own id, so the validator excludes
      // it explicitly; everything else in the set is genuinely earlier.
      validateRunnerStep(step.runner_id, step.config, key, sp, errors, warnings, {
        stepId: step.id,
        earlierStepIds: seenStepIds,
        hasContextCode:
          isObject(a.shared_context) && typeof a.shared_context.code === 'string' &&
          !!a.shared_context.code.trim(),
      });
